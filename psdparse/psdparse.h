...
protected:
  // 位置指定コンストラクタ (サブクラス VectorReader の clone 用)。
  MemoryReader(const uint8_t *base, int start, int end, int pos)
    : base_(base), start_(start), end_(end), pos_(pos) {}
public:
...
// VectorReader: MemoryReader that OWNS its bytes via a shared_ptr<vector>.
//
// MemoryReader references external storage; VectorReader is for freshly
// generated bytes (e.g. RLE-encoded channel data from set_layer_pixels /
// add_layer) that must live as long as any reader points into them. clone()
// shares the same buffer (shared_ptr), so copying a ChannelInfo (which clones
// its imageData) is safe — the bytes outlive every clone. The buffer is built
// once and never mutated, so base_ stays valid.
class VectorReader : public MemoryReader {
public:
  explicit VectorReader(std::shared_ptr<std::vector<uint8_t>> buf)
    : MemoryReader(buf && !buf->empty() ? buf->data() : (const uint8_t *)"",
                   0, buf ? (int)buf->size() : 0, 0),
      buf_(std::move(buf)) {}

  IteratorBase *clone() override { return make_(start_, end_, pos_); }
  IteratorBase *cloneOffset(int offset) override {
    int ns = pos_ + offset;
    if (ns < start_) ns = start_;
    if (ns > end_)   ns = end_;
    return make_(ns, end_, ns);
  }
  IteratorBase *cloneRange(int offset, int length) override {
    int ns = pos_ + offset;
    if (ns < 0)    ns = 0;
    ...
    if (ne > end_) ne = end_;
    if (ne < ns)   ne = ns;
    return make_(ns, ne, ns);
  }

private:
  VectorReader(std::shared_ptr<std::vector<uint8_t>> buf, int s, int e, int p)
    : MemoryReader(buf && !buf->empty() ? buf->data() : (const uint8_t *)"", s, e, p),
      buf_(std::move(buf)) {}
  IteratorBase *make_(int s, int e, int p) { return new VectorReader(buf_, s, e, p); }
